...
public class VHACD : MonoBehaviour
{
    ...
    {
        var deletables = transform.GetComponents<Collider>().Where(x => !x.isTrigger);
        foreach(var deletable in deletables)
        {
            DestroyImmediate(deletable);
        }
    ...
    public void Awake()
    {
        var colliders = transform.GetComponents<Collider>().Where(x => !x.isTrigger);
